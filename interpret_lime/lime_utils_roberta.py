...
SPECIAL_TOKENS = [101, 102, 100, 0]
# generate current batch of perturbations
def generate_perturbed_sentences(seed_sample, num_perturb, max_length, tokenizer, device):
	# initialize a bow form
	vocab = list(set( [token for token in seed_sample if token not in SPECIAL_TOKENS] ))
	#print(seed_sample)
	sentence_without_specials = [token for token in seed_sample if token not in SPECIAL_TOKENS]
	...
	print('inv data: ', len(inverse_data[0]))
	...
	for sampled_tokens in inverse_data:
		...
		sampled_tokens = [101] + sampled_tokens + [102] + [0]*num_pads
...
def lime_per_example(inputs, model, args, tokenizer):
		...
		batch_sequence, batch_attention, bow, mod_sentence, mapper = generate_perturbed_sentences(inputs['input_ids'].squeeze(0).tolist(), args.num_perturb, args.max_sent_len, tokenizer, args.device)
		...
		labels_column = np.zeros((neighborhood_labels.shape[0], int(neighborhood_labels.max())+1))
